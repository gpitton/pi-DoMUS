...
class NavierStokesExplicit
  :
  public PDESystemInterface<dim,spacedim,NavierStokesExplicit<dim,spacedim,LAC>, LAC>
{

public:
  virtual ~NavierStokesExplicit () {}
  NavierStokesExplicit (bool dynamic);
...
NavierStokesExplicit<dim,spacedim, LAC>::
NavierStokesExplicit(bool dynamic)
  :
  PDESystemInterface<dim,spacedim,NavierStokesExplicit<dim,spacedim,LAC>, LAC>(
    "Navier Stokes Interface",
    dim+1,
    3,
    "FESystem[FE_Q_Bubbles(2)^d-FE_DGQ(1)]",
    (dim==3)?"u,u,u,p":"u,u,p",
    "1,0"),
  AMG_A("AMG for A"),
  AMG_Ap("AMG for Ap"),
...
template <int dim, int spacedim, typename LAC>
void NavierStokesExplicit<dim,spacedim,LAC>::
declare_parameters (ParameterHandler &prm)
{
  PDESystemInterface<dim,spacedim, NavierStokesExplicit<dim,spacedim,LAC>,LAC>::
  declare_parameters(prm);
  this->add_parameter(prm, &prec_name,  "Preconditioner","cah-cha",
      ...
      for (unsigned int i=0; i<residual[0].size(); ++i)
        {
          ...
          // nonlinearity always on rhs
          // TODO: modify for Adams-Bashforth or Extrapolation methods
            nl_u = grad_u_old * u_old;

          //    ResidualType non_linear_term = 0;

          if (use_skew_symmetric_advection)
            res += 0.5 * ( scalar_product( nl_u, v) + scalar_product(grad_v * u_old, u) );
          else
            res += scalar_product( nl_u, v);

          double norm = std::sqrt(SacadoTools::to_double(u_old*u_old));

          if (norm > 0 && SUPG_alpha > 0)
            res += scalar_product( nl_u, SUPG_alpha * (h/norm) * grad_v  * u_old);
...
template <int dim, int spacedim, typename LAC>
void
NavierStokesExplicit<dim,spacedim,LAC>::compute_system_operators(
